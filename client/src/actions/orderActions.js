...
const payOrder = (orderResult) => async (dispatch,getState) => {
	try{
		dispatch({type:ORDER_PAY_REQUEST, payload:orderResult});
		const {userSignin:{userInfo}} = getState();
		const {data} = await axios.put('/api/orders/' + orderResult.order._id + '/pay/',orderResult.paymentResult, {headers:{Authorization:"Bearer " + userInfo.token}});
...
export { createOrder,listOrder,deleteOrder,detailsOrder,payOrder };